      ...
      <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-indigo-500 scrollbar-track-white/10 snap-x snap-mandatory">
        {recentlyViewed.slice(0, 10).map((memory) => (
          <div
            ...
            tabIndex={0}
            aria-label={`Memory: ${memory.aiData?.summary || memory.title}`}
          >
            <p className="font-semibold text-white mb-2 hover:text-indigo-400 transition-colors duration-300 truncate">
              {memory.aiData?.summary || memory.title || "Untitled Memory"}
            </p>

            {Array.isArray(memory.aiData?.tags) && memory.aiData.tags.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-2">
                {memory.aiData.tags.slice(0, 5).map((tag, idx) => (
                  <span
                    key={idx}
            ...
            <p className="text-gray-400 text-xs md:text-sm truncate">
              {new Date(memory.createdAt || memory.timestamp).toLocaleString()}
            </p>
          </div>
        ))}
      </div>
