...
const MemoryLane = () => {
  const { recentlyViewed, markRecentlyViewed } = useAppContext();
  const [searchTerm, setSearchTerm] = useState("");
  const [allMemories, setAllMemories] = useState([]); // all memories from backend
  ...
  return (
    <MemoryLaneErrorBoundary>
      <div className="relative min-h-screen flex flex-col px-6 py-10 bg-white/10 backdrop-blur-sm"
        ...
        <div className="flex-1 mt-6 relative">
          <AnimatePresence>
            {isLoading ? (
              ...
              <motion.p key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center mt-10 text-red-500 text-lg">
                {error}
              </motion.p>
            ) : searchTerm.trim() ? (
              searchResults.length > 0 ? (
                ...
                <motion.p key="no-results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center mt-10 text-gray-500 text-lg">
